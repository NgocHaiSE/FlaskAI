...
if int(camera_type) == 0:
    ...
    # cap = ffmpegcv.VideoCaptureStream(camera_link)
    cap = ffmpegcv.VideoCaptureStreamRT(camera_link)  # Low latency & recent buffered
    cap = ffmpegcv.ReadLiveLast(ffmpegcv.VideoCaptureStreamRT, camera_link) #no buffer
    print("Sử dụng RTSP URL.")
SCORE_THRESH = 0.5
IOU_THRESH = 0.5
...
while True:
    ret, frame = cap.read()
    if not ret:
        print("Lỗi: Không thể đọc khung hình, đang kết nối lại...")
        cap.release()
        time.sleep(1)  # Chờ trước khi kết nối lại
        cap = ffmpegcv.VideoCaptureStreamRT(camera_link)  # Low latency & recent buffered
        cap = ffmpegcv.ReadLiveLast(ffmpegcv.VideoCaptureStreamRT, camera_link) #no buffer
        if not cap.isOpened():
            print("Không thể kết nối lại với camera")
            break
        continue
    ...
    for face in faces:
        ...
        if not matched:
            ...
            if detected:
                ...
                # insert_featdetail(personIds[i], img_name)
                ...
                # send_notification(personIds[i], CAMERA_ID)
